...
#             "llm_type": "OpenAILLM",
#             "model": "gpt-3.5-turbo",
#             "openai_key": "",  # Empty string for testing, will use env var in production
#             "temperature": 0.7,
#             "max_tokens": 150,
#             "top_p": 0.9,
...
async def test_update_agent(client: httpx.AsyncClient, access_token: str):
    """Test updating an agent with improved error handling."""
    headers = {"Authorization": f"Bearer {access_token}"}
    agent_id = None
    try:
        ...
        # Test first with just description and tags update (minimal)
        try:
            minimal_update = {
            ...
            update_resp = await client.put(
                ...
                headers=headers, 
                json=minimal_update,
                timeout=15.0  # Increased timeout for this specific request
            )
            
            # If minimal update succeeds, we're done
            if update_resp.status_code == 200:
                print("Successfully updated agent with minimal payload")
                assert update_resp.json()["description"] == "Updated description"
                return
                
            # If we get here, the minimal update failed, try with full config
            print(f"Minimal agent update failed with status {update_resp.status_code}, trying with full config")
            
        except Exception as e:
            print(f"Minimal agent update attempt failed: {str(e)}")
        
        # Try with full config if minimal update didn't succeed
        full_update = {
            "description": "Updated description", 
            "tags": ["test", "updated"],
            "config": {
                ...
                "max_tokens": 200,   # Changed from 150
                "top_p": 0.9,
                "output_response": True,
                "prompt": "You are a very helpful assistant."  # Updated prompt
            }
        }
        ...
        update_resp = await client.put(
            ...
            headers=headers, 
            json=full_update,
            timeout=20.0  # Even longer timeout for full update
        )
        
        assert update_resp.status_code == 200, f"Failed to update agent, got status {update_resp.status_code}"
        assert update_resp.json()["description"] == "Updated description"
        
    except ReadTimeout:
        pytest.skip("Agent update request timed out - server may be busy")
    except Exception as e:
        pytest.fail(f"Agent update test failed: {str(e)}")
    finally:
        # Clean up
        if agent_id:
            try:
                ...
                print(f"Warning: Failed to clean up agent {agent_id}: {str(e)}")
...
#     """Test updating a workflow with improved error handling."""
#     headers = {"Authorization": f"Bearer {access_token}"}
#     agent_id = None
#     workflow_id = None
#     try:
...
#         # Test updating with just description and tags
#         try:
#             update_payload = {
...
#             }
#             update_resp = await client.put(
#                 urljoin(BASE_URL, f"workflows/{workflow_id}"), 
#                 headers=headers, 
#                 json=update_payload,
#                 timeout=15.0  # Extended timeout
#             )
            
#             assert update_resp.status_code == 200, f"Failed to update workflow, got status {update_resp.status_code}"
#             assert update_resp.json()["description"] == "Updated workflow description"
            
#         except ReadTimeout:
#             pytest.skip("Workflow update request timed out - server may be busy")
#         except Exception as e:
#             pytest.fail(f"Workflow update test failed: {str(e)}")
...
#                 await delete_workflow(client, headers, workflow_id)
#             except Exception as e:
#                 print(f"Warning: Failed to clean up workflow {workflow_id}: {str(e)}")
...
#     agents_to_create = [
#         {
#             "name": f"SearchAgent1_{uuid.uuid4().hex[:8]}", 
#             "description": "Search agent 1",
#             "tags": ["search", "test"], 
#             "config": {
#                 "llm_type": "OpenAILLM",
#                 "model": "gpt-3.5-turbo",
...
#         },
#         {
#             "name": f"SearchAgent2_{uuid.uuid4().hex[:8]}", 
#             "description": "Search agent 2",
#             "tags": ["search"], 
#             "config": {
#                 "llm_type": "OpenAILLM",
#                 "model": "gpt-3.5-turbo",
...
#         },
#         {
#             "name": f"FilterAgent_{uuid.uuid4().hex[:8]}", 
